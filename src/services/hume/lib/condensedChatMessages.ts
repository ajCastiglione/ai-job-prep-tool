...
export function condensedChatMessages(messages: Message[]) {
  return messages.reduce((acc, message) => {
    const data = _getChatEventData(message) ?? _getJsonMessageData(message);
...
function _getJsonMessageData(message: Message) {
...
function _getChatEventData(message: Message) {
